...
- [ ] W22 · **IMPL**, then **REVIEW** · Checklist v1.1 polish (developer's hands-on review vs live TRW app): group actions consolidated into a `…` bottom sheet (kills all `prompt()`), configurable daily reset time + "RESETS IN" countdown, Share→JSON export, Import→paste-text. Local-only, no schema bump, no new deps. → `prompts/W22-checklist-polish.md`
- [ ] W23 · **IMPL**, then **REVIEW** · ⛔ gated on W22 + a developer inspiration/dialogue session · Notes v1 as the second top tab in the Checklist hub (the deferred D4 notepad: folders/tags/5-star, connector-ready). Prompt written after the developer shares reference examples.
